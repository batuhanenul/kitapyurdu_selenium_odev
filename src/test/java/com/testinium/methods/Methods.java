...
public class Methods {
    ...
    public void addFavorites() {
        ...
        List<WebElement> products = findElements(By.cssSelector(".product-list .add-to-favorites"));
        Random rnd = new Random();

        for (int i = 1; i < 5; i++) {
            WebElement el = products.get(rnd.nextInt(products.size()));
            scrollWithAction(el);
            el.click();
            waitBySeconds(1);
        }
    ...
    public void favoritesCheck(){
        ...
        public void mouseOverLogOut(By by){
        WebElement webElement = findElement(by);
            actions.moveToElement(webElement).perform();
            click(findElement(By.linkText("Çıkış")));
        }
